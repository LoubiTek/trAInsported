...
function tutorial.start()
	...
	loadingScreen.addSubSection("Nouvelle carte", "Tutoriel 2: Gauche ou droite ?")
...
function tutorial.createTutBoxes()
	...
	local k = 1
	tutorialSteps[k] = {}
	tutorialSteps[k].stepTitle = "Où aller ?"
	tutorialSteps[k].message = "Bienvenue au deuxième tutoriel !\n\nIci, vous apprendrez:\n1)Comment gérer les jonctions\n2)Les bases Lua\n\nMalheureusement, nous allons devoir recourir à une théorie ici. Mais cela en vaut la peine quand nous arriverons aux tutoriels 3 et 4 !"
	tutorialSteps[k].buttons = {}
	tutorialSteps[k].buttons[1] = {name = "Démarrer tutoriel", event = nextTutorialStep}
	k = k + 1
	...
	tutorialSteps[k].buttons = {}
	tutorialSteps[k].buttons[1] = {name = "Retour", event = prevTutorialStep}
	tutorialSteps[k].buttons[2] = {name = "Plus d'informations", event = additionalInformation("The code will fail if you forget the line 'counter = 0', because before you can add 1 to the counter variable, it needs to be initialised. Since ai.init will always run BEFORE ai.chooseDirection, so the code will work."), inBetweenSteps = true}
	k = k + 1
	...
	tutorialSteps[k].buttons = {}
	tutorialSteps[k].buttons[1] = {name = "Retour", event = prevTutorialStep}
	tutorialSteps[k].buttons[2] = {name = "Plus d'informations", event = additionalInformation("1) A == B (are A and B equal?)\n2) A > B (A greater than B?)\n3) A <= B (Is A smaller or equal to B?)\n4) A ~= B (A does not equal B)"), inBetweenSteps = true}
	tutorialSteps[k].buttons[3] = {name = "Suivant", event = nextTutorialStep}
	k = k + 1
	...
	tutorialSteps[k].buttons[1] = {name = "Retour", event = prevTutorialStep}
	tutorialSteps[k].buttons[2] = {name = "Plus d'informations", event = additionalInformation("It will be sufficient to tell the trAIn to go east the first time and otherwise go south, because, when it's coming from the south and you tell it to go south, it will automatically default to north (because it can't go south)...\n\nThere's more than one solution to this. But be careful: Everything you do after a 'return' statement won't get executed, because when the game reaches the 'return', it will end the function."), inBetweenSteps = true}
...
function tutorial.roundStats()
	...
	love.graphics.print("Tutoriel 2: Gauche ou droite ?", x + roundStats:getWidth()/2 - FONT_STAT_MSGBOX:getWidth("Tutoriel 2: Gauche ou droite ?")/2, y+10)
